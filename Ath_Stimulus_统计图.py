...
def power_(epochs):
    ...
    power = tfr_morlet(epochs, freqs=freqs, n_cycles=n_cycles, use_fft=True,
                        return_itc=False, decim=1, n_jobs=1,average=True,picks = ['Pz','Cz'])
    return power
...
#threshold:min=-3.703541 max=5.861419
T_obs, clusters, cluster_p_values, H0 = permutation_cluster_1samp_test (
    power_data, n_permutations=n_permutations,
    threshold=3, tail=0,
    adjacency=adjacency,
...
plt.imshow((T_obs_plot[0]+T_obs_plot[1])/2, cmap=plt.cm.RdBu_r,
           extent=[-0.2,0.6,4,30],
           aspect='auto', origin='lower', vmin=-8, vmax=8)#+T_obs_plot[1]+T_obs_plot[2]+T_obs_plot[3])/4
plt.colorbar()
plt.xlabel('Time (ms)')
plt.ylabel('Frequency (Hz)')
plt.title('Ath_Stimulus')
